...
- [x] `feature_completeness.toml` rubric file in `.konjo/rubrics/`. *Correction (S13 Phase 0):* `refactor_safety.toml` and `security_audit.toml` also shipped here but had no code path loading them by name; deleted rather than left as dark claims — see `LEDGER.md`.
