    ...
    match cli.command {
        ...
        Commands::Learn(cmd) => match cmd {
            LearnCmd::List { limit, postmortem_only } => {
                commands::handle_learn_list(limit, postmortem_only, db_path()).await?;
            }
            LearnCmd::Show { id } => {
                commands::handle_learn_show(&id, db_path()).await?;
            }
            LearnCmd::Export { limit } => {
                commands::handle_learn_export(limit, db_path()).await?;
            }
        },

        // ── lopi schedules ──────────────────────────────────────
        Commands::Schedules(ScheduleCmd::List) => {
            commands::handle_schedules(&cfg)?;
        }
    }
