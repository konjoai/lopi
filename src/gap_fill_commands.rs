...
pub async fn run(
    ...
    let surface = match SpecSurface::load(&repo)? {
        Some(s) => {
            if !quiet { println!("  📋 spec surface: {} items (cached)", s.len()); }
            s
        }
        None => {
            let live = SpecSurface::extract(&repo)?;
            if !quiet { println!("  📋 spec surface: {} items (live)", live.len()); }
            live
        }
    };

    if surface.is_empty() {
        if !quiet { println!("  No spec items found. Run `lopi spec --save` first."); }
        return Ok(QualitySnapshot::empty(repo.to_string_lossy().to_string()));
    }

    // 2. Run tests.
    if !quiet { println!("  🧪 running tests…"); }
    ...
    if !quiet { println!("  ✅ {} results: {} passing, {} failing", results.len(), passing, failing); }
    ...
    let run_id = store.save_quality_run(QualityRunRecord {
    ...
    if !quiet {
        if let Ok(Some((latest, prev))) = store.quality_trend_delta(&repo.to_string_lossy()).await {
            let arrow = if latest > prev { "↑" } else if latest < prev { "↓" } else { "→" };
            println!("  📈 coverage: {:.0}% {arrow} (was {:.0}%)", latest * 100.0, prev * 100.0);
        } else {
            let score = if surface.is_empty() { 0.0 } else { passing as f64 / surface.len() as f64 };
    ...
    if gaps.is_empty() {
        if !quiet { println!("  ✅ No coverage gaps — all spec items have passing tests."); }
        return Ok(snapshot);
    }
    ...
    if dry_run {
        if !quiet { println!("  dry-run: not queuing fix tasks"); }
        return Ok(snapshot);
    }

    // 6. Queue fix tasks via the sail API.
    if !quiet { println!("  📤 queuing fix tasks on {sail_url}…"); }
    ...
    for gap in &gaps {
        let goal = format!(
            ...
            repo.display(), gap.description, gap.file, gap.line,
        );
        ...
        match client.post(&url).json(&body).send().await {
            Ok(resp) if resp.status().is_success() => {
                queued += 1;
                if !quiet { println!("     ↳ queued: {}", gap.name); }
            }
...
impl QualitySnapshot {
    fn empty(repo_path: String) -> Self {
        Self { run_id: String::new(), repo_path, spec_items: 0, passing: 0, failing: 0, gaps: 0 }
    }

    #[must_use]
    pub fn score(&self) -> f64 {
        if self.spec_items == 0 { 0.0 } else { self.passing as f64 / self.spec_items as f64 }
    }
}
...
pub async fn watch_loop(
    ...
    println!("🔄 lopi watch-gap-fill — {} every {interval_minutes} min", repo.display());
...
fn log_snapshot(snap: &QualitySnapshot) {
    ...
    println!(
        ...
        score * 100.0, snap.spec_items, snap.gaps,
        if snap.run_id.is_empty() { "—" } else { &snap.run_id[..8.min(snap.run_id.len())] }
    );
}
...
mod tests {
    ...
    fn snapshot_score_correct() {
        let s = QualitySnapshot {
            run_id: "x".into(), repo_path: "/r".into(),
            spec_items: 10, passing: 8, failing: 2, gaps: 2,
        };
