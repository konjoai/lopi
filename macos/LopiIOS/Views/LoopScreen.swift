...
/// Loop Engineering — the iOS counterpart of macOS's `LoopView.swift`
/// (Phase 4). Same read-mostly cockpit — effective `.lopi/loop.toml`, the
/// L1–L4 autonomy ladder, each schedule's trust level (the one writable
/// per-schedule control), discovered skills/rules, run traces, and the
/// Konjo quality gates — reflowed for a single portrait column: macOS's
/// 5/4/3/2-column grids become 2-column (or 1-column where content is
/// wordy), and the side-by-side Skills/Rules panels stack vertically.
/// `KonjoPanel`/`Sparkline`/`LoopModels` are reused directly from the shared
/// `Lopi/Theme`/`Lopi/Components`/`Lopi/Networking` sources — no port cost.
struct LoopScreen: View {
    ...
    private func healthPanel(_ h: LoopHealth) -> some View {
        KonjoPanel {
            VStack(alignment: .leading, spacing: 14) {
                ...
                let statCols = [GridItem(.flexible()), GridItem(.flexible())]
                LazyVGrid(columns: statCols, spacing: 10) {
                    statTile("Success", pct(h.stats.successRate), tint: rateColor(h.stats.successRate))
                    statTile("Verifier", h.stats.verifierTotal == 0 ? "—" : pct(h.stats.verifierPassRate), tint: Konjo.ice)
    ...
    private func statTile(_ label: String, _ value: String, tint: Color = Konjo.fg) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            Text(value).font(Konjo.sans(18, weight: .bold)).foregroundStyle(tint)
        }
    ...
    private func chartTile(_ title: String, _ value: String, _ samples: [Double], _ color: Color) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack {
                Text(title.uppercased()).font(Konjo.mono(8.5)).tracking(1).foregroundStyle(Konjo.fgMute)
                Spacer()
                Text(value).font(Konjo.mono(10, weight: .medium)).foregroundStyle(color)
            }
            Sparkline(samples: samples, color: color).frame(height: 36)
        }
    }
    ...
    private func runsPanel() -> some View {
        KonjoPanel {
            VStack(alignment: .leading, spacing: 12) {
                panelHead("Recent Runs", "tap a run for its attempt-by-attempt trace") { EmptyView() }
    ...
    private func runRow(_ r: LoopRun) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            Button {
                Haptics.tap()
                Task { await model.selectRun(r.taskId) }
            } label: {
                HStack(spacing: 10) {
                ...
                .frame(maxWidth: .infinity, alignment: .leading)
                .background(Konjo.bg2)
                .overlay(RoundedRectangle(cornerRadius: 8).stroke(isOpen ? Konjo.ice.opacity(0.5) : Konjo.line, lineWidth: 1))
                .clipShape(RoundedRectangle(cornerRadius: 8))
            }
            .buttonStyle(.plain)
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 5) {
                ForEach(Array(stages.enumerated()), id: \.offset) { i, st in
                    Text(st.uppercased()).font(Konjo.mono(7.5)).tracking(0.8).foregroundStyle(Konjo.fgMute)
                    if i < stages.count - 1 {
                        Text("→").font(Konjo.mono(7.5)).foregroundStyle(Konjo.fgMute.opacity(0.5))
                    }
                }
            }
            HStack(spacing: 12) {
    ...
    private func configPanel(_ c: LoopConfigDTO) -> some View {
        KonjoPanel {
            VStack(alignment: .leading, spacing: 14) {
                ...
                let cols = [GridItem(.flexible(), alignment: .topLeading), GridItem(.flexible(), alignment: .topLeading)]
                LazyVGrid(columns: cols, alignment: .leading, spacing: 14) {
                    field("Default Autonomy", "\(c.autonomyTag) · \(c.autonomyLabel)", tint: levelColor(c.autonomyTag))
                ...
                LazyVGrid(columns: cols, spacing: 10) {
                    ForEach(levels) { l in
                        VStack(alignment: .leading, spacing: 3) {
                            ...
                            Text(ladderHint(l)).font(Konjo.mono(9.5)).foregroundStyle(Konjo.fgMute)
                        }
                        .padding(10)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .background(Konjo.bg2)
    ...
    // MARK: Self-prompting strategy

    private func selfPromptPanel(_ snap: LoopSnapshot) -> some View {
        KonjoPanel {
            VStack(alignment: .leading, spacing: 12) {
                panelHead("Self-Prompting Strategy", "how the loop re-prompts itself after a failed attempt") {
                    strategyMenu(snap)
                }
    ...
    private func strategyMenu(_ snap: LoopSnapshot) -> some View {
        Menu {
            ForEach(snap.selfPromptStrategies) { st in
                Button {
                    Haptics.selection()
                    focusedStrategy = st.value
                    Task { await model.setLoopStrategy(st.value) }
                } label: {
                    if st.value == snap.config.selfPrompt {
                        Label("\(st.tag) · \(st.label)", systemImage: "checkmark")
                    } else {
                        Text("\(st.tag) · \(st.label)")
                    }
                }
            }
        } label: {
            HStack(spacing: 4) {
                Text(snap.selfPromptStrategies.first { $0.value == snap.config.selfPrompt }
                    .map { "\($0.tag) · \($0.label)" } ?? snap.config.selfPrompt)
                    .font(Konjo.mono(11)).foregroundStyle(Konjo.fg).lineLimit(1)
    ...
    private func escalationRow(_ c: LoopConfigDTO) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            Toggle(isOn: Binding(
                ...
                set: { newValue in Haptics.selection(); Task { await model.setLoopEscalation(newValue) } }
            )) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    HStack(spacing: 6) {
                        ForEach(Array(c.escalationLadder.enumerated()), id: \.element.id) { idx, rung in
                            ...
                            HStack(spacing: 4) {
                                ...
                                Text(rung.tag).font(Konjo.mono(10, weight: .bold)).foregroundStyle(strategyColor(rung.tag))
                            }
                            .padding(.horizontal, 8).padding(.vertical, 4)
                            .background(Konjo.bg2)
                            .overlay(RoundedRectangle(cornerRadius: 6).stroke(Konjo.line2, lineWidth: 1))
    ...
    private func strategyCard(_ st: LoopSelfPromptOption, active: Bool) -> some View {
        Button {
            ...
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(Konjo.bg2)
            .overlay(RoundedRectangle(cornerRadius: 8).stroke(active ? Konjo.ice : Konjo.line, lineWidth: active ? 1.5 : 1))
            .clipShape(RoundedRectangle(cornerRadius: 8))
        }
        .buttonStyle(.plain)
    }
    ...
    private func previewStrategy(_ snap: LoopSnapshot) -> LoopSelfPromptOption? {
        ...
        return snap.selfPromptStrategies.first { $0.value == target } ?? snap.selfPromptStrategies.first
    }
        ...
        KonjoPanel {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if snap.schedules.isEmpty {
                    Text("No scheduled loops — add one from the Schedule tab.")
                        .font(Konjo.mono(12)).foregroundStyle(Konjo.fgMute)
                } else {
    ...
    private func scheduleRow(_ s: LoopSchedule, _ levels: [LoopAutonomyOption]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 10) {
                Circle().fill(s.enabled ? Konjo.jade : Konjo.fgMute).frame(width: 8, height: 8)
                VStack(alignment: .leading, spacing: 2) {
            ...
            Menu {
                ForEach(levels) { l in
                    Button {
                        Haptics.selection()
                        Task { await model.setScheduleAutonomy(s.id, level: l.value) }
                    } label: {
                        if l.value == s.autonomyLevel {
                            Label("\(l.tag) · \(l.label)", systemImage: "checkmark")
                        } else {
                            Text("\(l.tag) · \(l.label)")
                        }
                    }
                ...
                HStack(spacing: 4) {
                    Text("\(s.autonomyTag) · \(s.autonomyLabel)")
                        .font(Konjo.mono(11)).foregroundStyle(levelColor(s.autonomyTag)).lineLimit(1)
                    Image(systemName: "chevron.down").font(.system(size: 8)).foregroundStyle(Konjo.fgMute)
                }
                .padding(.horizontal, 8).padding(.vertical, 4)
                .overlay(RoundedRectangle(cornerRadius: 6).stroke(Konjo.line2, lineWidth: 1))
        ...
        KonjoPanel {
            VStack(alignment: .leading, spacing: 10) {
                ...
                if rules.isEmpty {
                    ...
                    let cols = [GridItem(.adaptive(minimum: 100), spacing: 8, alignment: .leading)]
    ...
    private func panelHead<Trailing: View>(
        ...
        HStack(alignment: .firstTextBaseline) {
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(subtitle.uppercased()).font(Konjo.mono(9)).tracking(1.2).foregroundStyle(Konjo.fgMute)
            }
            Spacer()
            trailing()
        }
