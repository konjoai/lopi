...
/// kill switch. The iOS counterpart of macOS's `BudgetView.swift` — same
/// data/logic (`model.liveAgents`, `model.budgetBreaches`, `model.cancelTask`,
/// the `UserDefaults`-backed cap), reflowed for a single portrait column: the
/// stat row becomes a 2x2 grid instead of 4 cards in one `HStack` (macOS's
/// row doesn't fit a phone-width screen), everything else was already a
/// vertical `ScrollView` and needs no layout change. Presented as a sheet from
/// `StackOverviewScreen`, matching `ServerConfigScreen`'s chrome.
struct BudgetScreen: View {
    @Environment(AppModel.self) private var model
    @State private var cap: Double = BudgetScreen.loadCap()
    @State private var samples: [(t: Date, cost: Double)] = []

    private let tick = Timer.publish(every: 2, on: .main, in: .common).autoconnect()
    private let presets: [Double] = [1, 5, 10, 25, 50]
    private let statGrid = [GridItem(.flexible()), GridItem(.flexible())]
    ...
    private var stopAllRow: some View {
        Button { Haptics.warning(); stopAll() } label: {
            Label("Stop all running (\(running))", systemImage: "stop.fill")
                .font(Konjo.sans(13, weight: .semibold))
                .foregroundStyle(Konjo.rose)
                .frame(maxWidth: .infinity)
                .padding(.vertical, 10)
        }
        .buttonStyle(.plain)
        .overlay(RoundedRectangle(cornerRadius: 8).stroke(Konjo.rose.opacity(0.4), lineWidth: 1))
        .disabled(running == 0)
        .opacity(running == 0 ? 0.4 : 1)
    }

    // MARK: Stat cards — 2x2 grid (macOS's 4-wide row doesn't fit portrait)
    ...
    private func stat(_ label: String, _ value: String, _ c: Color) -> some View {
        VStack(alignment: .leading, spacing: 5) {
            ...
            Text(value).font(Konjo.sans(18, weight: .semibold)).foregroundStyle(c).monospacedDigit()
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(14)
        .background(Color.white.opacity(0.03), in: RoundedRectangle(cornerRadius: 12))
        .overlay(RoundedRectangle(cornerRadius: 12).stroke(Konjo.line2, lineWidth: 1))
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 8) {
                ...
                ForEach(presets, id: \.self) { p in
                    Button { Haptics.selection(); setCap(p) } label: {
                        Text("$\(Int(p))")
                            .font(Konjo.mono(10.5, weight: .semibold))
                            .foregroundStyle(cap == p ? Konjo.ice : Konjo.fgDim)
                            .padding(.horizontal, 8).padding(.vertical, 4)
                            .overlay(Capsule().stroke(cap == p ? Konjo.ice.opacity(0.5) : Konjo.line2, lineWidth: 1))
                    }
                    .buttonStyle(.plain)
                }
                Stepper("", value: Binding(get: { cap }, set: { Haptics.selection(); setCap($0) }), in: 0.5...200, step: 0.5)
                    .labelsHidden()
            }
        }
        .padding(16)
    ...
    private func spenderRow(_ a: LiveAgent, maxCost: Double) -> some View {
        HStack(spacing: 10) {
            ...
            Text(String(format: "$%.4f", a.costUsd))
                .font(Konjo.mono(11)).foregroundStyle(Konjo.flame).monospacedDigit().frame(width: 58, alignment: .trailing)
            Button { Haptics.warning(); Task { await model.cancelTask(a.id) } } label: {
    ...
    /// Every entry in `model.budgetBreaches` (up to 5) — mirrors macOS/web's
    /// "recent breaches" panel.
...
/// Burn-rate band relative to the cap. `Lopi/Views/Admin` (where macOS's own
/// `BudgetState` lives) isn't part of `LopiIOS`'s shared sources — see
/// `project.yml`'s `LopiIOS` target — so this is a small hand-kept copy
/// rather than a cross-target import.
private enum BudgetState { case ok, warn, over }
