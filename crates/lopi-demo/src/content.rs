...
pub const REPOS: [RepoTemplate; 4] = [
    RepoTemplate {
        ...
        description:
            "Order-processing API — axum + sqlx, deployed as a fleet of stateless workers.",
    },
    RepoTemplate {
        name: "lumen-console",
...
pub const GOALS: &[GoalTemplate] = &[
    ...
    GoalTemplate {
        repo_index: 0,
        goal: "fix the flaky retry test in the checkout handler",
    },
    GoalTemplate {
        repo_index: 0,
        goal: "add rate limiting to the /webhooks ingress endpoint",
    },
    GoalTemplate {
        repo_index: 0,
        goal: "migrate the audit_log table to a JSONB payload column",
    },
    GoalTemplate {
        repo_index: 0,
        goal: "fix the connection pool leak under sustained load",
    },
    GoalTemplate {
        repo_index: 0,
        goal: "add idempotency keys to the /orders POST endpoint",
    },
    GoalTemplate {
        repo_index: 0,
        goal: "reduce p99 latency on the inventory lookup query",
    },
    GoalTemplate {
        repo_index: 0,
        goal: "backfill missing tracing spans in the payment module",
    },
    GoalTemplate {
        repo_index: 0,
        goal: "fix the panic on malformed webhook signatures",
    },
    // lumen-console (1) — TypeScript web app
    GoalTemplate {
        repo_index: 1,
        goal: "fix the stale cache bug on the orders table filter",
    },
    GoalTemplate {
        repo_index: 1,
        goal: "backfill missing type exports in the SDK client package",
    },
    GoalTemplate {
        repo_index: 1,
        goal: "add keyboard navigation to the bulk-actions toolbar",
    },
    GoalTemplate {
        repo_index: 1,
        goal: "fix the layout shift on the dashboard's first paint",
    },
    GoalTemplate {
        repo_index: 1,
        goal: "migrate the legacy REST calls to the generated client",
    },
    GoalTemplate {
        repo_index: 1,
        goal: "add a loading skeleton to the customer detail panel",
    },
    GoalTemplate {
        repo_index: 1,
        goal: "fix the timezone bug in the audit trail timestamps",
    },
    // tidepool-etl (2) — Python data pipeline
    GoalTemplate {
        repo_index: 2,
        goal: "reduce the nightly ETL job's memory footprint under 2GB",
    },
    GoalTemplate {
        repo_index: 2,
        goal: "fix the duplicate-row bug in the order-events dedup step",
    },
    GoalTemplate {
        repo_index: 2,
        goal: "add a dead-letter queue for malformed ingest records",
    },
    GoalTemplate {
        repo_index: 2,
        goal: "speed up the warehouse backfill script past the 6h SLA",
    },
    GoalTemplate {
        repo_index: 2,
        goal: "fix the schema drift check against the analytics warehouse",
    },
    GoalTemplate {
        repo_index: 2,
        goal: "add retry-with-backoff to the upstream API fetch step",
    },
    // wayfarer-infra (3) — Infra
    GoalTemplate {
        repo_index: 3,
        goal: "add a health-check probe timeout to the ingress controller",
    },
    GoalTemplate {
        repo_index: 3,
        goal: "fix the Terraform drift on the staging security groups",
    },
    GoalTemplate {
        repo_index: 3,
        goal: "add autoscaling limits to the aurora-api worker pool",
    },
    GoalTemplate {
        repo_index: 3,
        goal: "rotate the staging cluster's expiring TLS certificates",
    },
    GoalTemplate {
        repo_index: 3,
        goal: "fix the Helm chart values that silently ignore --set overrides",
    },
];
...
pub const PATTERNS: &[(&str, &str)] = &[
    (
        "flaky retry test",
        "add explicit backoff jitter and re-run the suite 3x before scoring",
    ),
    (
        "rate limiting webhook",
        "reuse the existing token-bucket middleware rather than a bespoke limiter",
    ),
    (
        "memory footprint etl",
        "stream batches instead of loading the full frame into memory",
    ),
    (
        "terraform drift",
        "run `terraform plan` before touching any `.tf` file, never trust cached state",
    ),
    (
        "type exports sdk",
        "regenerate from the OpenAPI spec instead of hand-editing generated types",
    ),
    (
        "timezone bug",
        "store and compare timestamps in UTC, format at the display boundary only",
    ),
];
