...
/// every changed path is docs-only, *and* — via `Iterator::all` being
/// vacuously true on an empty slice — when nothing changed at all (e.g. an
/// attempt that halted before writing anything). Both cases mean there is no
/// source change to actually verify; running the real build check anyway
/// against a target repo with no compilable code produces a false `pass=0%`
/// failure rather than the honest "nothing to check" this is.
fn should_skip_build_check(changed: &[(bool, String)]) -> bool {
    changed.iter().all(|(_, path)| is_docs_path(path))
}
