...
fn evict_phase_does_not_remove_conclusion() {
    ...
    window.push(msg("raw discovery data", Phase::Discovery, PinPolicy::Never)).unwrap();
    ...
    assert!(ids.contains(&conclusion_id), "conclusion must survive evict_phase");
    assert!(
        window.turns().iter().all(|t| t.is_conclusion || t.phase != Phase::Discovery),
...
fn evict_to_budget_zero_does_not_remove_conclusion() {
    ...
    window.push(msg("evictable", Phase::Implementation, PinPolicy::BudgetEvictable)).unwrap();
    let conclusion_id = window.pin_conclusion("phase summary".to_string(), Phase::Implementation);

    window.evict_to_budget(0).unwrap();

    let ids: Vec<_> = window.turns().iter().map(|t| t.id).collect();
    assert!(ids.contains(&conclusion_id), "conclusion must survive evict_to_budget(0)");
}
...
fn conclusion_survives_phase_transition_of_its_own_phase() {
    ...
    window.push(msg("discovery noise", Phase::Discovery, PinPolicy::Never)).unwrap();
...
fn non_conclusion_always_pinned_also_requires_force() {
    ...
    let id = window.push(TaggedMessage {
        id: Uuid::new_v4(),
        role: Role::User,
