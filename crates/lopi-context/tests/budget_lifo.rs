...
fn oldest_turns_evicted_first_under_budget_pressure() {
    ...
    let id1 = window.push(msg_tokens("m1", 40, PinPolicy::BudgetEvictable)).unwrap();
    let id2 = window.push(msg_tokens("m2", 40, PinPolicy::BudgetEvictable)).unwrap();
    window.push(msg_tokens("m3", 40, PinPolicy::BudgetEvictable)).unwrap();
    window.push(msg_tokens("m4", 40, PinPolicy::BudgetEvictable)).unwrap();

    // pressure = 160/200 = 0.80 > 0.75: next push auto-evicts oldest turns.
    window.push(msg_tokens("m5", 40, PinPolicy::BudgetEvictable)).unwrap();
...
fn always_pinned_turns_never_evicted_by_budget() {
    ...
    let pinned_id = window.push(msg_tokens("pinned system", 100, PinPolicy::Always)).unwrap();
    window.push(msg_tokens("evictable 1", 50, PinPolicy::BudgetEvictable)).unwrap();
    window.push(msg_tokens("evictable 2", 50, PinPolicy::BudgetEvictable)).unwrap();
    ...
    assert!(ids.contains(&pinned_id), "Always-pinned turn must not be evicted by budget pressure");
...
fn budget_full_returns_error_when_nothing_evictable() {
    ...
    window.push(msg_tokens("pinned 1", 50, PinPolicy::Always)).unwrap();
    window.push(msg_tokens("pinned 2", 50, PinPolicy::Always)).unwrap();
...
fn explicit_evict_to_budget_frees_oldest_first() {
    ...
    let id_old = window.push(msg_tokens("oldest", 100, PinPolicy::BudgetEvictable)).unwrap();
    let id_new = window.push(msg_tokens("newest", 100, PinPolicy::BudgetEvictable)).unwrap();
