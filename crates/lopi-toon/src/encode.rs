...
impl Delimiter {
    pub(crate) fn ch(self) -> char {
        match self { Self::Comma => ',', Self::Tab => '\t', Self::Pipe => '|' }
    }
...
impl Default for EncoderOptions {
    fn default() -> Self { Self { indent: 2, delimiter: Delimiter::Comma } }
}
...
pub fn encode_with(value: &Value, opts: &EncoderOptions) -> String {
    let mut enc = Encoder { opts, buf: String::new() };
...
pub fn encode_task_context(
    ...
    let allowed_v: Vec<Value> = allowed.iter().map(|s| Value::String(s.to_string())).collect();
    let forbidden_v: Vec<Value> = forbidden.iter().map(|s| Value::String(s.to_string())).collect();
    let constraints_v: Vec<Value> = constraints.iter().map(|s| Value::String(s.to_string())).collect();
    ...
    if !patterns.is_empty() {
        let rows: Vec<Value> = patterns.iter().map(|(kw, c)| {
...
impl<'a> Encoder<'a> {
    ...
    fn write_root(&mut self, value: &Value) {
        ...
        match value {
            ...
            other              => {
                self.buf.push_str(&encode_scalar_value(other, self.opts.delimiter));
                self.buf.push('\n');
            }
        }
    }
    ...
    fn write_named_array(&mut self, indent: &str, key: &str, arr: &[Value], depth: usize) {
        ...
        if n == 0 {
            ...
            self.buf.push_str(&format!("[0{sym}]:\n"));
            return;
        }
        ...
        if arr.iter().all(is_primitive) {
            let vals: Vec<String> = arr.iter()
                .map(|v| encode_cell(v, d))
                .collect();
            let inline = vals.join(&d.ch().to_string());
            self.buf.push_str(indent);
            self.buf.push_str(key);
            self.buf.push_str(&format!("[{n}{sym}]: {inline}\n"));
            return;
        }
        ...
        if let Some(fields) = tabular_fields(arr) {
            ...
            self.buf.push_str(indent);
            self.buf.push_str(key);
            self.buf.push_str(&format!("[{n}{sym}]{{{fields_hdr}}}:\n"));
            let row_indent = " ".repeat((depth + 1) * self.opts.indent);
            for item in arr {
                let obj = item.as_object().unwrap();
                let cells: Vec<String> = fields.iter()
        ...
        self.buf.push_str(indent);
        self.buf.push_str(key);
        self.buf.push_str(&format!("[{n}{sym}]:\n"));
        for item in arr {
    ...
    fn write_list_item(&mut self, item: &Value, depth: usize) {
        ...
        match item {
            ...
            Value::Object(map) => {
                ...
                match fv {
                    Value::Array(arr) => {
                        ...
                        let n = arr.len();
                        if n == 0 {
                            self.buf.push_str(&format!("{k0}[0{sym}]:\n"));
                        } else if arr.iter().all(is_primitive) {
                            let vals: Vec<String> = arr.iter().map(|v| encode_cell(v, d)).collect();
                            self.buf.push_str(&format!("{k0}[{n}{sym}]: {}\n",
                                vals.join(&d.ch().to_string())));
                        } else {
                    ...
                    Value::Object(_) => {
                        self.buf.push_str(&format!("{k0}:\n"));
                        self.write_field(fk, fv, depth + 2);
                    }
                    scalar => {
                        let v = encode_scalar_value(scalar, self.opts.delimiter);
                        self.buf.push_str(&format!("{k0}: {v}\n"));
                    }
                }
                ...
                for (k, v) in entries {
                    ...
                    match v {
                        ...
                        scalar => {
                            ...
                            self.buf.push_str(&format!("{extra_indent}{ek}: {sv}\n"));
                        }
                    }
                }
            }
            ...
            scalar => {
                let v = encode_scalar_value(scalar, self.opts.delimiter);
                self.buf.push_str(&format!("{indent}- {v}\n"));
            }
        }
    }
...
fn tabular_fields(arr: &[Value]) -> Option<Vec<String>> {
    ...
    for item in arr {
        ...
        if obj.keys().cloned().collect::<Vec<_>>() != keys { return None; }
        // All values are primitives.
        for v in obj.values() {
            if !is_primitive(v) { return None; }
        }
    }
    Some(keys)
}
...
pub(crate) fn encode_key(k: &str) -> String {
    if key_is_safe(k) { k.into() } else { quote_string(k) }
}
...
fn quote_if_needed(s: &str, delim: Delimiter, in_array: bool) -> String {
    if must_quote(s, delim, in_array) { quote_string(s) } else { s.into() }
}
...
fn must_quote(s: &str, delim: Delimiter, in_array: bool) -> bool {
    ...
    for ch in s.chars() {
        ...
        if ch == delim.ch() && !in_array { return true; }
    }
    // Contains the active array delimiter.
    if in_array && s.contains(delim.ch()) { return true; }
    false
}
...
fn is_numeric_like(s: &str) -> bool {
    ...
    if i < b.len() && b[i] == b'.' {
        i += 1;
        if i >= b.len() || !b[i].is_ascii_digit() { return false; }
        while i < b.len() && b[i].is_ascii_digit() { i += 1; }
    }
