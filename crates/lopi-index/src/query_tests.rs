...
async fn find_ranks_and_bounds_results() {
    ...
    store
        .replace_file_symbols(
            ...
            "h",
            &[
                sym(0, "run_agent", "Runs the agent."),
                sym(1, "stop", "Stops it."),
            ],
        )
        .await
    ...
    let result = find(&store, "repo", "run", None, None, None, 10)
        .await
        .unwrap();
...
async fn find_reports_truncation_honestly() {
    ...
    store
        .replace_file_symbols("repo", "src/lib.rs", Language::Rust, "h", &syms)
        .await
        .unwrap();
...
async fn read_returns_exact_span_by_qualified_name() {
    ...
    s.line_end = 4;
    store
        .replace_file_symbols("repo", "lib.rs", Language::Rust, "h", &[s])
        .await
        .unwrap();
