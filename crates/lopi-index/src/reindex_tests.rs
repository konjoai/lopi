...
async fn zero_symbol_file_does_not_get_reindexed_forever() {
    ...
    assert_eq!(
        first.files_indexed, 2,
        "both files parsed on the first pass"
    );
    ...
    assert_eq!(
        ...
        "only the genuinely-edited file is reparsed"
    );
}
...
async fn clean_tree_one_file_commit_reindexes_only_that_file() {
    ...
    assert_eq!(
        ...
        "committed one-file change: no repo-wide hash sweep"
    );
}
