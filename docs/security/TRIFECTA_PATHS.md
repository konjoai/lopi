...
Verified against: `b93e68f` · 2026-07-29 (re-verified; G0 flagged this doc stale — 20+ commits
past `e2f9362` — after this unrelated Sprint S13 Phase 0 PR landed a commit on top of it, even
though this PR touches none of the files this doc cites. Diffed every commit `e2f9362..b93e68f`
against every cited file: `crates/lopi-core/src/config.rs`, `crates/lopi-remote/src/whatsapp.rs`,
...
Of the six changed files, confirmed directly against the current tree: two real line-citation
drifts, both fixed below — `src/cli.rs`'s `Sail`/`ServeWebhooks` `--host` citations (§0) and
`config.rs`'s `WebConfig.host` citation (§4). No content-level drift in any of the six: `whatsapp.rs`'s
`/task` handler citation (§1, §6 row K) is unchanged (Sprint E's `/cost` addition landed after it,
not before); `pool/`'s new `economics_admit.rs`/`runaway_monitor.rs` gate budget admission, not
tool access or trust classification, so §1's "full tool access" claim is unaffected; `web/mod.rs`'s
one-line change adds `/api/economics` to the `protected` router — live-confirmed §7's structural
fix (every route lives in `protected` or the public fallback, no third place) still holds.
(Superseded prior banner covered `e2f9362` and below; the Finding #4 symbol-index, Sprint E
`/cost`, and demo-mode reconciliation history it recorded still holds — nothing here revisits it.)
§0's other citations and §2's citations describe the pre-Sprint-S2 baseline at `3a8a2ff` by design
and are left as historical record — see §5 for what actually shipped)
