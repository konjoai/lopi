...
## Sprint S13, Phase 0 (Quality-claim honesty pass, stopped per stop rule)

**Decision (rubric deletion is a one-way door on the documentation contract, not just a file removal).** `.konjo/rubrics/refactor_safety.toml` and `security_audit.toml` are deleted because an exhaustive grep of every `.rs` file under `crates/`/`src/` found zero call sites passing either name to `verifier::load_rubric_file`/`resolve_rubric` — they were documented as shipping (`KONJO_VERIFIER.md`, `PLAN.md`) but never reachable. The one-way door: any future rubric must have **both** the `.toml` file under `.konjo/rubrics/` **and** a real call site passing its name before a doc may claim it ships. A rubric file with no caller is a dark rubric again, and this sprint's own audit is the second time (after `KONJO_VERIFIER.md`/`PLAN.md` overclaimed it) that this exact failure mode has cost an audit pass to catch — treat "wired" as meaning grep-verified, not "the file exists in the directory."

**Decision (CLAUDE.md hard-rule bullets describe the real gate, not the aspirational one, even when the real gate is weaker).** 3 of 8 "Additional Hard Rules" bullets (coverage ≥80%/95%, zero undocumented public APIs, function body ≤50 lines) had no `konjo-gate.yml` step that could actually fail a build — either `continue-on-error: true` or no mechanical check at all, only a WARNING-tier LLM review question. These were corrected in place to state what genuinely enforces (a lower coverage floor; nothing, for doc coverage and function length) rather than either left inaccurate or newly wired live. **One-way door: wiring any of these three back to hard enforcement is real Wave 1 scope (tracked separately, not part of S13) — until that lands, `CLAUDE.md` must keep describing the weaker real gate, not the aspirational one**, because the failure this sprint exists to close is exactly a doc that oversells its own enforcement.

**Decision (indexing-site count in the baseline table is not currently reproducible to an exact number).** Three different, individually-defensible filter definitions for "production raw-index sites" (`[0]`/`[1]` outside test code) produced 341, 194, and 185 respectively — none matched the brief's stated 202, and neither did a second independent audit run against the identical commit. This is flagged, not resolved, in `.konjo/killtests/S13/PHASE0-STOP-RULE.md` §1. **One-way door for whoever seeds Phase 2's indexing floor: pick one exact, scriptable definition (which directories/filename patterns count as "test"), write it into `index_floor_check.py` itself, and treat that script's own output as the only number that matters going forward** — do not try to reconcile it against this sprint's baseline table number, which was never pinned down precisely enough to be a ground truth.

**Stop-rule verdict (why this sprint has no Phase 1–4).** The brief's own pre-flight rule: more than 3 self-claims with no enforcing step found in Phase 0 stops the sprint before any new gates are added on top of an inaccurate inventory. This audit found 5 (2 dark rubrics + 3 unenforced hard-rule bullets). **This is the second independent audit to reach that exact tally against the same base commit** (`b93e68f`) — see `.konjo/killtests/S13/PHASE0-STOP-RULE.md` §6 for the relationship to the sibling run on `claude/s13-quality-substrate-e7e92y` (PR #182). Resuming this sprint past Phase 0 should re-run the baseline table (numbers may have moved since) and confirm the tally is still ≤3 before starting Phase 1 — not skip straight to Phase 1 on the strength of this entry alone.
