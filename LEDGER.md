...
patch-level change (`0.25.0` → `0.25.1`), not a breaking one. Worth stating plainly
specifically *because* the previous entry below is the opposite case, and a reader
skimming version history for "what's a safe upgrade" should be able to tell the two
apart without re-reading both changelogs in full.
