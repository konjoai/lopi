...
Scoring → OpeningPr → RollingBack`) attributed to an `AgentState` enum.
That enum still exists in `crates/lopi-core/src/agent.rs` but has no
`OpeningPr`/`RollingBack` variants (`Idle`/`Planning`/`Implementing`/
`Testing`/`Scoring`/`Done`/`Errored`) and, as of this writing, is
constructed nowhere in the codebase — it's unused scaffolding, not what
actually surfaces through the CLI or API. `TaskStatus` above is the real,
live status type; treat it as ground truth over the `LOPI_VS_OPENCLAW.md`
table until that doc is corrected.
